...
最近看到serv00的webmail邮箱（理论上其他webmail也行，比如站内的，至于serv00邮箱怎么创建可以站内搜搜），想着可不可以利用他批量发件给客户，于是创建了这个项目，项目利用 GitHub Actions 实现自动化批量发送邮件。你只需要维护一个收件人列表，修改邮件内容，然后手动触发，即可轻松完成邮件的批量发送。
